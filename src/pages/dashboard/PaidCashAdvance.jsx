...
const PaidCashAdvance = () => {
    ...
    const url = baseURL +`/paid-cashadvance/`;
    // const urlimg = imgURL;  
    ...
    useEffect(() => {
      ...
      const handlePageChange = async (url) => {
        try {
          ...
          setnextUrl(data.next)
          setviewCashAdvance(data.results);
          // setuserProfile(data.results.user.profile);
          setLoading(false);
        } catch (error) {
          console.error(error);
        }
                    ...
                    <div className="shadow md:overflow-hidden overflow-x-auto border-b border-gray-200 sm:rounded-lg">
                      ...
                      {viewCashAdvance.map((item,i) => (
                          <tr>
                            ...
                            <td className="px-6 py-4 whitespace-nowrap  ">
                              <p className="text-sm font-medium text-gray-900">{item.user.profile.department}</p>
                            </td>
...
export default PaidCashAdvance;