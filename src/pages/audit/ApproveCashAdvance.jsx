...
const handleApprovalChange = (event) => {
  ...
  const handleSubmit = async (event) => {
    ...
    const url = baseURL +`/audited-cash-advances/` +id;
    ...
    if (response.status === 200) {
        ...
        // console.log(data)
        navigate("/auditdashboard/cashadvance"); 
        setLoading(false);
            ...
            <div className="flex flex-wrap w-[30rem] mx-2 mb-6">
                    <div className="w-full justify-center  px-3 mb-6 md:mb-0">
                         <label className=" flex uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-gender">
                            Cash Advance Status: {viewCashAdvance.is_approved === 'audit' ? <span className="pb-2 mb-2 pl-2 text-xs text-green-600">Approved</span>  :  
                            <span className="pl-2 pb-2 mb-2 text-xs text-red-600">Not Approved</span> }
                        </label>

                        <div className="relative md:w-1/2 mt-2">
                            <select value={isApproved} onChange={handleApprovalChange} className="block   appearance-none w-full bg-gray-200 border border-gray-200 text-gray-700 py-3 px-4 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-gray-500" placeholder={viewCashAdvance.is_approved === 'audit' ? 'Approved'  :  'Not Approved'} required id="grid-gender">
                                   
                                    <option>Select</option>
                                    <option value="audit">Approved</option>
                                    <option value="account">Not Approved</option>
...
export default ApproveCashAdvance;